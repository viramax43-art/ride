...
function FitInitialRideBounds({ requests }: { requests: RideRequest[] }) {
  const map = useMap()
  const hasFitted = useRef(false)

  useEffect(() => {
    if (hasFitted.current || requests.length === 0) return
    const points = requests.flatMap((request) => [
      [request.from.latlng.lat, request.from.latlng.lng] as [number, number],
      [request.to.latlng.lat, request.to.latlng.lng] as [number, number],
    ])
    if (points.length === 0) return
    hasFitted.current = true
    map.fitBounds(L.latLngBounds(points), { padding: [64, 64], maxZoom: 13 })
  }, [map, requests])
  ...
  return (
    <main className={`flex-1 relative ${hasBottomPanel ? 'admin-map--panel-open' : ''}`}>
      ...
      >
        ...
        <FitInitialRideBounds requests={visibleActiveRequests} />
